...
def export_articulated_tractor_model():
    useCollocation = True
    Nx = 6
    Nu = 3
    ...
    def ode_control_tractor_front(x, u):
        '''
        x[0]: xf
        x[1]: yf
        x[2]: theta_r
        ...
        return np.array([u[0] * np.cos(x[2] + x[3] + x[5]),
                         u[0] * np.sin(x[2] + x[3] + x[5]),
                         theta_r_dot,
                         u[1],
    ...
    if useCollocation:
        ode_casadi = mpc.getCasadiFunc(ode_control_tractor_front, [Nx, Nu], ["x", "u"], funcname="f")
    else:
        ode_casadi = mpc.getCasadiFunc(ode_control_tractor_front, [Nx, Nu], ["x", "u"], rk4=True, Delta=Delta, M=1)
