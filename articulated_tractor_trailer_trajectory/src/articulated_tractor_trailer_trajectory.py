...
from export_articulated_tractor_trailer_model import *
...
def start():
    ...
    rospy.init_node('articulated_tractor_trailer', anonymous=True)
...
# Wait until the first odometry message is available.
    while ((REAR_P3D is None) or (TRAILER_P3D is None) or (ARTICULATION_ANGLE is None) or (FL_STEER_ANGLE is None) or (FR_STEER_ANGLE is None)) and not rospy.is_shutdown():
        rospy.sleep(.1)
    ...
    while not rospy.is_shutdown() and t<Nsim:
        ...
        """ [xt, yt, theta_r, gamma, theta_r - theta_t, phi] """
        _xk = np.array([trailer_curr_x,
                        trailer_curr_y,
                        rear_curr_yaw,
                        articulation_angle,
